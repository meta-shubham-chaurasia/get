package com.assignment.EAD9.controllers;
import java.io.File;
...
import com.assignment.EAD9.dtos.EmployeeDto;
import com.assignment.EAD9.models.Employee;
import com.assignment.EAD9.models.Vehicle;
import com.assignment.EAD9.services.EmployeeService;
import com.assignment.EAD9.services.ImageService;
...
public class EmployeeController {
	...
	public String searchImage(int empId) {
		File folder = new File("/EAD9/src/main/resources/static/images");
		String imagePath = "http://localhost:9090/images/blank.png";
		for (File f: folder.listFiles()) {
			if (f.getName().equals(empId + ".png")) {
				imagePath = "http://localhost:9090/images/" + f.getName();
			}
		}
		return imagePath;
	}
	...
	public String uploadImage(@RequestParam("image") MultipartFile imageFile, HttpSession session) {
		try {
			...
			session.setAttribute("imagePath", "http://localhost:9090/images/" + empId + ".png");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "redirect:display";
